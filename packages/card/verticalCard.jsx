...
import { titleSize, typeIcon } from "./cardUtilities.jsx";
...
* @constructor
*/
export default function VerticalCard({ data , cardSize, showDescriptionOnMobile, hideImages}) {
    const {
        title,
        description,
        liveUrl,
    ...
    const descriptionOnMobile = cardSize === "small" && showDescriptionOnMobile === false ? " su-hidden lg:su-block " : "";

    return (
        <article className="su-component-card su-relative su-w-full">
            { 
                (hideImages === false && imageUrl) && (
            ...
            <div className="su-flex su-flex-wrap su-gap-[9px]">
                <h2 className={ `su-w-full ${ titleSize(cardSize) } su-leading-[25.2px] lg:su-leading-[28.8px] su-font-serif su-my-0` }>
                    <a
                        href={liveUrl}
                ...
                <p className={ `su-mb-0${ descriptionOnMobile }su-text-[19px] su-leading-[23.75px]` }>
                    { description }
                </p>
            </div>
        </article>
